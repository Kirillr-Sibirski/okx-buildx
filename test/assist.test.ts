...
test("assist defaults to the guided doctor flow with strict policy", () => {
  const result = interpretAssistRequest("Check my wallet health on X Layer");

  assert.equal(result.intent, "doctor");
...
test("assist prompt asks for strict json output", () => {
  ...
  assert.match(prompt, /\"intent\": \"doctor \| review \| status \| inspect \| plan \| report \| execute\"/);
});
